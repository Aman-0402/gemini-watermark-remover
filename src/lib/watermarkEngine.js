...
// ── Combined removal pipeline (multi-select modes) ──
// settings.modes is an array, e.g. ['unblend'], ['strong', 'blur'], or all
// five at once. Exactly one "math" mode runs (strong takes priority over
// unblend if both are selected, since it's the stronger of the two), then
// any selected overlay modes stack on top in a fixed order: blur, then
// pixelate, then blackout. Used identically by the live tuner preview and
// the image export path (both work with a full-frame ImageData + canvas).
export function applyMathStep(bgImg, imageData, width, height, base, settings) {
  const wm = resolveBox(base, width, height, settings);
  const roi = getRoi(width, height, wm);
  const modes = settings.modes && settings.modes.length ? settings.modes : ['unblend'];
  const gain = settings.gain ?? 1;
  const copy = new ImageData(new Uint8ClampedArray(imageData.data), width, height);

  if (modes.includes('strong')) {
    removeWatermarkUniform(copy, wm, gain);
  } else if (modes.includes('unblend')) {
    const alpha = buildAlpha(bgImg, roi, wm, gain);
    removeWatermark(copy, alpha, { x: roi.x, y: roi.y, width: roi.width, height: roi.height });
  }
...
export class VideoWatermarkEngine {
  ...
  async process(file, opts = {}) {
    ...
    const modes = opts.modes && opts.modes.length ? opts.modes : ['unblend'];
    const runsUnblend = modes.includes('unblend') && !modes.includes('strong');
    const base = opts.mode === 'gemini' ? getWatermarkInfo(width, height) : this.getVeoWatermark(width, height);
    const wm = resolveBox(base, width, height, opts);
    const roi = runsUnblend ? getRoi(width, height, wm) : null;
    const alpha = runsUnblend ? buildAlpha(this.engine.bg96, roi, wm, gain) : null;
    const region = runsUnblend ? { x: 0, y: 0, width: roi.width, height: roi.height } : null;
